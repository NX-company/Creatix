...
- Для логотипов: max-width 150-200px, для иллюстраций: max-width 100%`
  ...
  const modernDesign = getModernDesignPrompt(docType, styleConfig)
  
  const prompt = `Создай ПОЛНЫЙ, СОВРЕМЕННЫЙ HTML документ на основе этого контента:
${content}
...
- Создай ПОЛНЫЙ HTML документ от <!DOCTYPE html> до </html>
- Используй встроенные CSS стили (inline styles и <style> в <head>)
- Современный дизайн 2025 (градиенты, тени, отступы, красивая типографика)
- Адаптивный дизайн (media queries)
- На русском языке
- Для изображений используй ТОЛЬКО плейсхолдеры IMAGE_0, IMAGE_1, IMAGE_2 и т.д.
- НЕ ОСТАНАВЛИВАЙСЯ на середине - создай документ ДО КОНЦА
- Размести ВСЕ изображения согласно правилам выше

НАЧИНАЙ ГЕНЕРАЦИЮ ПОЛНОГО HTML ДОКУМЕНТА:`
